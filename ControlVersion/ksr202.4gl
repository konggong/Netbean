#===========================================================================
# SCCS ID         : @(#) ksr202.4gl 1.8@(#) 16/02/18 19:17:03
# Program Name    : KSR202.4GL Daily Unit-In-Set Control Report (TACCBAL)
# Programmer Name : kyj
# Parameters      : @Date
# Paper Size      : 15cpi 195 col
# KHIS Version 1.8  : ntp 18/02/16 PR-20160217-00004 : add column Remark,Check by,Authorize
# KHIS Version 1.7  : pww 24/09/15 PR-20150528-00001 : Available to run reports far back.
...
#---------------------------------------------------------------------------
  ...
  format
     page header
          ...
          print column   1,KN_REPORT_LINE[1,150]
     ...
     before group of r.type
       case
         ...
         when r.type = '5' print column  1,"** ตราสารหนี้ **"
       end case
     ...
     on every row
        if flagdat = 0 then
	     ...
	     --skip 1 line
             ...
             let LINE1 = ks_prformat(LINE1,6,"........................................")
             print column 1 ,LINE1 clipped
       end if

     page trailer
          print column   1,KN_REPORT_LINE[1,150]
          print column   1,KN_REPORT_TRAILER[1,150]
          print column   1,KN_REPORT_LINE[1,150]

     on last row
          print column   1,KN_REPORT_END
	  skip 2 line
	  print column   9,"_____________________               _____________________"
	  print column  15,"Check by                            Authorize"
 ...
 define r_sharecode  like mlsd.sharecode,
   ...
   format
      page header
         ...
         print column   1,"As At : ",kd_bankdate(in.date1,-1) using "dd/mm/yyyy"
         print column   1,KN_REPORT_LINE[1,140]
         print column   1,HEAD1 clipped
         print column   1,HEAD2 clipped
         print column   1,KN_REPORT_LINE[1,140]
     ...
     before group of r.sharecode
      ...
      page trailer
         print column   1,KN_REPORT_LINE[1,140]
         print column   1,KN_REPORT_TRAILER[1,140]
         print column   1,KN_REPORT_LINE[1,140]
...
if in.showbyloc = 0 then
   ------------------------------------------ showbyloc = 0 -----------------------------------------------------
   let DETAIL1 = "^1                   ^2                    ^3                   ^4                  ^5                   ^6"
   #               xxxxxxxxx1xxxxxxxx2 -9,999,999,999,999    xxxxxxxxx1xxxxxxxxx2 -9,999,999,999,999  -9,999,999,999,999   xxxxxx
   let HEAD1   = "|--------------- ^1     ---------------|  |---------------   SDC   ---------------|"
   let HEAD2   = " Sharecode                         Unit    Sharecode                          Unit           Diff Unit   Remark"
else
   ------------------------------------------ showbyloc = 1 -----------------------------------------------------
   --t LINE1   = "----------------------------------------------------------------------------------------------"
   let HEAD1   = "                                   |----- ^1     -----| |-----  SDC  -----|                   "
   let HEAD2   = "Sharecode             Location                    Unit                Unit           Diff Unit    Remark"
   --t LINE1   = "----------------------------------------------------------------------------------------------"
   #             "xxxxxxxxx1xxxxxxxxx2  xxxxxxxxx1xx  -9,999,999,999,999  -9,999,999,999,999  -9,999,999,999,999    xxxxxx"
   let DETAIL1 = "^1                    ^2            ^3                  ^4                  ^5                    ^6"
   --t LINE1   = "----------------------------------------------------------------------------------------------"
end if
